...
async function agregarSufijo() {
  ...
  try {
    ...
    for (const archivo of archivos) {
      ...
      // Obtenemos el nombre base (sin extensión) y la extensión
      const extension = path.extname(archivo); // Obtiene ".pdf"
      const nombreBase = path.basename(archivo, extension); // Obtiene "nombre_del_archivo"
      ...
      if (extension === extensionObjetivo && !nombreBase.endsWith(sufijo)) {
        ...
        // Ej: "nombre_del_archivo" + " OCT" + ".pdf"
        const nombreNuevo = `${nombreBase}${sufijo}${extension}`; 
