<template>
  <v-container fluid>
    ...
    <v-row justify="center" class="text-center my-4">
      <v-col cols="12">
        <h1 class="text-h4 font-weight-bold text-grey-darken-3">Gestión de Características de Producto</h1>
        ...
        <v-card-text class="pt-4">
          <v-container>
            <v-row>
                ...
                <v-card variant="outlined" class="sticky-form">
                  ...
                  <v-card-text>
                    ...
                    <template v-if="modalType === 'categoria'">
                      <v-text-field v-model="formState.nombre_categoria_producto" label="Nombre de la categoría *"
                        variant="outlined" density="compact" ref="firstInput"></v-text-field>
                      <v-textarea v-model="formState.descripcion_categoria" label="Descripción (opcional)"
                        variant="outlined" density="compact" rows="3"></v-textarea>
                    </template>

                    <template v-if="modalType === 'proveedor'">
                      <v-text-field v-model="formState.nombre_proveedor" label="Nombre del proveedor *"
                        variant="outlined" density="compact" ref="firstInput"></v-text-field>
                      <v-text-field v-model="formState.telefono_proveedor" label="Teléfono *" variant="outlined"
                        density="compact"></v-text-field>
                      <v-text-field v-model="formState.razon_social_proveedor" label="Razón Social" variant="outlined"
                        density="compact"></v-text-field>
                      <v-text-field v-model="formState.direccion_proveedor" label="Dirección" variant="outlined"
                        density="compact"></v-text-field>
                      <v-text-field v-model="formState.email_proveedor" label="Email" type="email" variant="outlined"
                        density="compact"></v-text-field>
                      <v-text-field v-model="formState.nit_proveedor" label="NIT" variant="outlined"
                        density="compact"></v-text-field>
                      <v-checkbox v-model="formState.activo" label="Activo"></v-checkbox>
                    </template>

                    <template v-if="modalType === 'material'">
                      <v-text-field v-model="formState.descripcion_material" label="Descripción del material *"
                        variant="outlined" density="compact" ref="firstInput"></v-text-field>
                    </template>

                    <template v-if="modalType === 'modelo'">
                      <v-text-field v-model="formState.descripcion_modelo" label="Descripción del modelo *"
                        variant="outlined" density="compact" ref="firstInput"></v-text-field>
                    </template>
                    <template v-if="modalType === 'armazon'">
                      <v-text-field v-model="formState.descripcion_modelo" label="Descripción del modelo *"
                        variant="outlined" density="compact" ref="firstInput"></v-text-field>
                    </template>

                    <template v-if="modalType === 'color'">
                      <v-text-field v-model="formState.nombre_color" label="Nombre del color *" variant="outlined"
                        density="compact" ref="firstInput"></v-text-field>
                      <v-text-field v-model="formState.codigo_hex" label="Código Hexadecimal" variant="outlined"
                        density="compact">
                        <template v-slot:prepend-inner>
                          ...
                          <v-icon
                            :style="{ backgroundColor: formState.codigo_hex, borderRadius: '4px', marginRight: '8px', border: '1px solid #ccc' }" />
                        </template>
                      </v-text-field>
                    </template>
                  </v-card-text>
                  <v-card-actions class="px-4 pb-4">
                    <v-btn color="primary" variant="flat" @click="guardarItem">{{ editId ? 'Actualizar' : 'Guardar'
                      }}</v-btn>
              ...
              <!-- Tabla a la derecha -->
              <v-col cols="12" md="8">
                <v-data-table-server :headers="modalConfig ? modalConfig.headers : []" :items="data"
                  :items-length="data.length" :loading="isLoading"
                  :item-value="modalConfig ? modalConfig.primaryKey : ''" density="compact" class="elevation-1">
                  ...
                  <template v-slot:item.actions="{ item }">
                    ...
                    <v-icon icon="mdi-delete" color="red"
                      @click="confirmarEliminacion(item[modalConfig.primaryKey])"></v-icon>
                  </template>
                </v-data-table-server>
              </v-col>
            </v-row>
...
const managementCards = [
  ...
  { type: 'armazon', title: 'Armazón', icon: 'mdi-safety-goggles' },
